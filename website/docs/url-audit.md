...
:::note
This page is regenerated by the GitHub Actions URL-audit workflow. Automated reachability does not imply dataset license compatibility, clinical validity, deidentification adequacy, or permission to reuse data. `credentials_or_client_required` means the official platform expects credentials, a configured client, or a browser-mediated request; it is an access requirement, not a dataset download failure.
:::
