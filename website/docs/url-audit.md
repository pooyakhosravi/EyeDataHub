...
:::note
This page is regenerated by the GitHub Actions URL-audit workflow. Automated reachability does not imply dataset license compatibility, clinical validity, deidentification adequacy, or permission to reuse data. `no_bot_allowed` means the source host blocked automated probes even though manually checked browser links were healthy; it should not be interpreted as a dead source URL.
:::
